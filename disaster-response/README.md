...
3. (Not necessary if the `model\dr_classifier.pkl` is already available). To run the ML pipeline that trains and deploys the model, change the directory to `models` and run this command: `python train_classifier.py DisasterResponse.db dr_classifier.pkl`. (It is assumed that the database is located under the `data` directory.)
