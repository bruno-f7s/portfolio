...
* Download the complete project or fork this repository to your local computer.

### For Windows User:
1. To run the ETL pipeline that cleans, processes and stores the data, run this command from the main directory:
    ```
      python data\\process_data.py data\\messages.csv data\\categories.csv data\\DisasterResponse.db
    ```
2. To run the ML pipeline that trains and deploys the model, run this command from the main directory:
    ```
      python models\\train_classifier.py data\\DisasterResponse.db models\\dr_classifier.pkl
    ```
3. To start the app run the following command inside the `app` directory:
    ```
      python run.py
    ```
4. Open this link on your browser: [http://127.0.0.1:3001/](http://127.0.0.1:3001/).
...
2. To run the ML pipeline that trains and deploys the model, run this command from the main directory:
    ```
      python models/train_classifier.py data/DisasterResponse.db models/dr_classifier.pkl
    ```
3. To start the app run the following command inside the `app` directory:
    ```
      python run.py
